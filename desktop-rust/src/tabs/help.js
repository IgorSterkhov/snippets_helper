...
const i18n = {
  en: {
    ...
    finance_desc: 'Monthly planning module for recurring expenses. Create plan cards such as Regular monthly, choose one currency per plan, and maintain a nested expense tree with direct amount, note, and aggregate total columns. Parent totals are calculated as the parent direct amount plus all descendant amounts, so groups can either only aggregate children or also include their own fixed cost. Plans and tree rows can be reordered by drag-and-drop; rows can also be nested by dropping into another row. Finance data is desktop-local in this first version and is not yet synced to API or mobile.',
  ...
  ru: {
    ...
    finance_desc: 'Модуль месячного планирования регулярных расходов. Создавайте карточки планов, например Regular monthly, задавайте одну валюту на план и ведите вложенное дерево расходов с колонками прямой суммы, заметки и агрегированного итога. Итог родителя считается как собственная сумма родителя плюс суммы всех потомков: группа может только агрегировать детей или дополнительно иметь свою фиксированную сумму. Планы и строки дерева можно менять местами drag-and-drop; строки также можно вкладывать друг в друга drop-ом внутрь другой строки. В первой версии Finance-данные локальны для десктопа и еще не синхронизируются с API или мобильным приложением.',
