...
const i18n = {
  en: {
    ...
    ai_name: 'AI Agent',
    ai_desc: 'Ask DeepSeek from a dedicated AI tab without storing the DeepSeek token on desktop or mobile; the server owns the provider key and validates the safe command schema. Chat mode answers in the tab, while Command mode can open Tasks, Notes, and Snippets, create tasks, and add or complete task checkboxes through local app commands so normal sync propagates the changes. Desktop voice prompts reuse the configured Whisper/Deepgram flow; mobile voice prompts use Android speech recognition and require the APK build with microphone permission. Telegram bot support is server-side, deny-by-default, and only works for chats explicitly bound to a user by a server console command.',
