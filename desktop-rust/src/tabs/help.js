...
const i18n = {
  en: {
    ...
    shortcuts_desc: 'Store and quickly access text snippets. Two-panel layout with searchable list and tabbed detail view for Code, Description, Links, Note, and Related snippets. Optional tabs appear only when content exists; if only Code is available, the tab strip is hidden. Filter by colored tags (glob patterns). Key Cloud derives stable colored keys from underscore-separated snippet names, shows their frequency in a packed zoomable cloud with Fit, pan, adaptive labels, full-key tooltips, persistent cache, and no-cache progress while the layout is rebuilt; clicking a key runs search for it while clearing the tag filter. Related ranks snippets by shared keys. Rendered Markdown code blocks have copy buttons, compact language headers, and tolerate leading spaces before triple-backtick fences. The editor toolbar can insert inline or fenced code blocks. Links open explicitly in the browser or a separate app window. Description is collapsed by default while editing to leave more room for code. Expandable card preview in the list. Markdown auto-rendering. Obsidian note integration. Copy values instantly with Enter.',
