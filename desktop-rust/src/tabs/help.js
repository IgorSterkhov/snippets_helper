...
const i18n = {
  ...
  ru: {
    ...
    clickhouse_desc: 'Локальный справочник ClickHouse в группе DEV. Модуль хранит курируемый набор официальных raw Markdown документов ClickHouse в локальной SQLite базе и разбивает большие страницы на секции/функции. Страницы открываются как легкое оглавление секций, активная страница раскрывается в левой навигации, а выбор секции рендерит только этот блок. Поиск тоже работает на уровне секций/функций, поэтому большие страницы функций возвращают нужный блок, а не всю статью. Кнопка Update docs вручную обновляет локальный кэш из официального репозитория документации ClickHouse и показывает нативный прогресс с процентом, оставшимися страницами, временем выполнения, итоговой сводкой, временем последнего обновления и счетчиками страниц/секций. Changelog показывает добавленные, измененные, удаленные или неудачно загруженные источники последних обновлений.',
