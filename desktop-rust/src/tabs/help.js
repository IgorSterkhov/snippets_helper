...
const i18n = {
  en: {
    ...
    shortcuts_desc: 'Store and quickly access text snippets. Two-panel layout with searchable list and tabbed detail view for Code, Description, Links, Note, and Related snippets. The left panel can sort snippets by name (A-Z) or modified date (newest first), and remembers the selected mode. The top strip can switch between colored tag filters and synced pinned snippet chips; snippets can be pinned from the detail header, renamed chips update automatically, and pinned chips can be reordered by drag-and-drop. The detail header can create, copy, preview, and revoke live public share links for snippets. Public snippet pages render Markdown-like value and description safely while preserving copyable plain-code snippets. The Markdown toolbar can upload server-optimized images or paste clipboard screenshots into snippet value or description and render them as Figure Cards in previews, mobile, and public share pages; desktop preview uses a native media fallback so server images display even when the WebView blocks remote image loading. The image preview shows the current variant name and supports previous/next buttons plus keyboard arrows. Image upload errors open a persistent modal with copyable diagnostics for reporting failed uploads, processing, or previews. Optional tabs appear only when content exists; if only Code is available, the tab strip is hidden. Filter by colored tags (glob patterns). Key Cloud derives stable colored keys from underscore-separated snippet names, shows their frequency in a packed zoomable cloud with Fit, pan, adaptive labels, full-key tooltips, persistent cache, selectable Dense/Fast layout algorithms, and no-cache progress while the layout is rebuilt; clicking a key runs search for it while clearing the tag filter. Related ranks snippets by shared keys. Rendered Markdown code blocks have copy buttons, compact language headers, and tolerate leading spaces before triple-backtick fences. The editor toolbar can insert inline or fenced code blocks. Links open explicitly in the browser or a separate app window. Description is collapsed by default while editing to leave more room for code. Expandable card preview in the list. Markdown auto-rendering. Obsidian note integration. Copy values instantly with Enter.',

    notes_name: 'Notes',
    notes_desc: 'Organize notes in nested folders (tree view with arbitrary depth). Built-in Markdown editor with toolbar and live preview. The toolbar can upload server-optimized images, paste clipboard screenshots, and insert portable Markdown image links that render as Figure Cards on desktop, mobile, and public share pages; desktop preview uses a native media fallback so server images display even when the WebView blocks remote image loading. The image preview shows the current variant name and supports previous/next buttons plus keyboard arrows. Image upload errors open a persistent modal with copyable diagnostics for reporting failed uploads, processing, or previews. Notes can be shared through live public links that can be created, copied, previewed, and revoked from desktop or mobile; creating a note link saves current editor content and syncs it first, and the public note page renders safe Markdown. Expandable card previews in the note list. Auto markdown rendering on open. Pin important notes to the top and reorder pinned note chips by drag-and-drop.',

    ai_name: 'AI Agent',
    ai_desc: 'Ask DeepSeek from a dedicated AI tab without storing provider tokens on desktop or mobile; each sync API user saves their own server-side DeepSeek key and Telegram bot token in Settings > AI, and saved secrets are never shown back in the app. Settings can check the current user DeepSeek balance, open the DeepSeek usage cabinet, show a Telegram pairing command, poll the bot for that pairing message, list bound chats, and unbind chats without server console access. Chat mode answers in the tab, while Command mode can open Tasks, Notes, and Snippets, create tasks, and add or complete task checkboxes through local app commands so normal sync propagates the changes. Command mode can make a short follow-up pass after search-only plans, so requests such as finding a task and marking one checkbox done can continue without asking again. The AI tab help button explains Chat vs Command mode, Telegram behavior, voice input, and example prompts. Desktop voice prompts have a Voice selector for local Whisper or Deepgram live transcription; Deepgram uses the local Whisper settings key/model and inserts the stopped live transcript into the AI prompt. Mobile voice prompts use Android speech recognition and require the APK build with microphone permission. Telegram bot support is server-side, deny-by-default, and uses the DeepSeek key and Telegram bot token of the paired app user.',

    tasks_name: 'Tasks',
    tasks_desc: 'Personal task manager with hierarchical checkboxes (up to 3 levels), customizable categories and statuses with colors, draggable pinned chip strip, drag-and-drop card→dropdown to change category/status, checklist drag-and-drop that respects hidden completed items, persisted collapse state for checklist branches, inline keyboard editing (Enter creates a same-level checkbox directly after the current item; Tab/Shift+Tab change nesting), Markdown notes per task, Tracker link + auxiliary links, card background palette, completed items hidden by default, and 1-column, 2-column, and Focus view layouts. Focus view keeps filters and pinned chips visible, shows a searchable task index on the left, and opens the selected task on the right. Full sync with the server and mobile app.',
