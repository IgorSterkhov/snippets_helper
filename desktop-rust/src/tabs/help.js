...
const i18n = {
  ...
  ru: {
    ...
    launchpad_desc: 'Ctrl+Alt+Space открывает frameless окно Launchpad поверх остальных окон для быстрого доступа с клавиатуры. Оно умеет открывать модули в отдельных окнах, открывать конкретные задачи, заметки и сниппеты внутри отдельных окон модулей, а также запускать Exec-команды прямо из микропанели с компактным статусом и выводом. В меню шестеренки можно включать Search и Recent, добавлять модули или конкретные объекты через drill-down Add item и входить в Edit Launchpad; Ctrl+E переключает режим редактирования, Escape сначала выходит из edit/status-режима и только потом закрывает окно. Раскладка Launchpad, recent и глобальный hotkey хранятся локально для текущей машины; hotkey меняется в Settings > Shortcuts и применяется после перезапуска приложения.',
