...
const i18n = {
  en: {
    ...
    shortcuts_desc: 'Store and quickly access text snippets. Two-panel layout with searchable list and tabbed detail view for Code, Description, Links, and Note. Optional tabs appear only when content exists; if only Code is available, the tab strip is hidden. Filter by colored tags (glob patterns). Rendered Markdown code blocks have copy buttons, compact language headers, and tolerate leading spaces before triple-backtick fences. The editor toolbar can insert inline or fenced code blocks. Links open explicitly in the browser or a separate app window. Description is collapsed by default while editing to leave more room for code. Expandable card preview in the list. Markdown auto-rendering. Obsidian note integration. Copy values instantly with Enter.',
  ...
  ru: {
    ...
    shortcuts_desc: 'Храните и быстро используйте текстовые сниппеты. Двухпанельный интерфейс с поиском и детальным просмотром во вкладках Code, Description, Links и Note. Дополнительные вкладки появляются только когда в них есть содержимое; если доступен только Code, строка вкладок скрыта. Фильтрация по цветным тегам (glob-паттерны). В Markdown-блоках кода есть кнопка копирования, компактный header с языком и поддержка ведущих пробелов перед triple-backtick fence. Кнопка кода в редакторе умеет вставлять inline-код и fenced code block. Ссылки открываются явно: в браузере или в отдельном окне приложения. Description при редактировании по умолчанию свернут, чтобы оставить больше места для кода. Раскрываемые карточки для быстрого предпросмотра. Авто-рендеринг Markdown. Интеграция с Obsidian. Копирование по Enter.',
