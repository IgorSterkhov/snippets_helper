...
const i18n = {
  ...
  ru: {
    ...
    search_desc: 'Поиск по локальным git-репозиториям: по имени файла (glob), по содержимому (ripgrep если установлен), по истории git. Git-поиск проверяет и сообщения коммитов, и измененные строки патча, поэтому можно найти изменения кода без совпадения в тексте коммита. Добавляйте именованные цветные репозитории, группируйте их, выбирайте где искать и проверяйте scope badge: All, конкретная группа или Ungrouped. Вкладки групп и чипы репозиториев можно менять местами drag-and-drop; чипы репозиториев по-прежнему можно перетаскивать на вкладку группы для переноса между группами. Результаты группируются по файлу с превью контекста по клику. Вкладка выгружается из памяти при неиспользовании.',
