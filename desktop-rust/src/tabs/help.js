...
const i18n = {
  en: {
    ...
    launchpad_desc: 'Ctrl+Alt+Space opens a frameless always-on-top Launchpad window for quick keyboard-first access. It can open detached module windows, open specific tasks, notes, snippets, or finance lists inside detached module windows, and run Exec commands directly with a compact status/output view. The + button adds modules, browsed module objects, containers, and separators; the gear menu toggles Search/Recent, sets grid columns and rows, and enters Edit Launchpad mode. In Edit mode, drag tiles, containers, separators, and container contents; drop tiles into or out of containers; resize containers from the lower-right handle or width/height fields; removing a container unwraps its contents back into the grid. Opening a module or object closes Launchpad, while commands keep it open and show their output. Ctrl+E toggles edit mode, Escape leaves edit/status modes before closing the window. Launchpad layout, recent items, and the global hotkey are stored per machine; change the hotkey in Settings > Shortcuts and restart the app to re-register it.',
    ...
    telegraph_share_desc: 'The Share dialog can publish notes and snippets to Telegra.ph as Telegram-friendly snapshot pages. The sync API prepares the safe Telegra.ph content and stores page metadata, while the desktop native app performs the direct Telegra.ph publish/update so publishing still works when the server cannot reach Telegra.ph. Because Telegra.ph has no table nodes, narrow Markdown tables up to 32 display columns stay aligned and monospaced. Tables with non-empty rows become labeled vertical records when they are wider or contain links, with only safe targets made clickable; header-only or empty tables stay compact. Failures open a persistent dialog with copyable diagnostics. Interactive HTML Cards degrade to safe links back to the sandboxed HTML asset.',
    ...
    finance_facts_name: 'Finance Facts import',
    finance_facts_desc: 'The Finance module has a global Facts mode for importing T-Bank CSV bank exports. Facts uses its own date-filter sidebar instead of the list picker, with filters for an exact date, date range, month, or year so large exports stay manageable; the month filter uses an in-app picker that opens reliably in the desktop WebView. The Facts registry has a visible all-fields search box on desktop and mobile for dates, descriptions, bank fields, MCC/card, amounts, lock/mapping state, and Finance targets. Import preview shows new and duplicate rows, expenses, income, date range, and currencies before commit. CSV import failures open a persistent copyable error dialog; parser errors include the column/value and raw CSV row when available, and T-Bank rows with an empty payment date fall back to the operation date. Imported facts are deduplicated by source/fingerprint, stored separately from plan rows, and can be mapped manually or through ordered mapping rules. Fact mapping and rule creation use a compact hierarchical item selector where parent/group rows are visible but only terminal rows can be selected. The fact Map modal can create a prefilled rule from the current fact and immediately apply it to existing unmapped facts. Rule matching supports bank category, description, MCC, direction, and amount ranges; applying rules can update only unmapped facts by default or explicitly remap assigned unlocked facts, and existing rules can be selected, edited, saved, and re-applied from the rules dialog. Each fact has a rules lock so manual assignments can be protected from future rule runs. Facts mapped to rows that later become groups are marked with a red alert and can be isolated through the Group target filter whenever such mappings exist globally; clicking Group target resets date/search filters before showing those facts. When a terminal row with direct facts or payment-calendar entries is turned into a group, Finance asks to move those direct targets to the new terminal child. Facts, allocations, mapping rules, and import batches sync through the API; mobile assignments are retried until the server accepts them, the mobile Sync details sheet shows pending rows, last debug data, and recent row-level sync history, and the mobile Facts screen has a persistent Cards/Compact view switch for wide or foldable screens. Public Finance shares include only safe assigned fact details for the shared list: date, description, bank category, MCC, amount, and assigned item.',

    sql_name: 'SQL Tools',
    sql_desc: 'A suite of SQL utilities with five sub-tools: Parser extracts table names and ClickHouse dictGet-family dictionary targets from queries, Analyzer generates DDL analysis using templates, Macrosing wraps queries in Jinja macros, Formatter beautifies SQL code by expanding multi-item WITH blocks, SELECT lists, top-level WHERE/PREWHERE/HAVING conditions, and ClickHouse SETTINGS options, and Obfuscator replaces sensitive values with placeholders.',
  ...
  ru: {
    ...
    sidebar_groups_desc: 'Developer-модули сгруппированы под компактной кнопкой DEV в левой панели. Finance, DEV, SQL и Superset используют разные цветные значки без дополнительных плиток и рамок. SQL, Superset, Commits, Search и ClickHouse раскрываются под DEV по клику или когда один из них активен, включая переход через Ctrl+Tab или команды приложения. При переходе в другой модуль группа снова сворачивается.',
    ...
    launchpad_desc: 'Ctrl+Alt+Space открывает frameless окно Launchpad поверх остальных окон для быстрого доступа с клавиатуры. Оно умеет открывать модули в отдельных окнах, открывать конкретные задачи, заметки, сниппеты и финансовые списки внутри отдельных окон модулей, а также запускать Exec-команды прямо из микропанели с компактным статусом и выводом. Кнопка + добавляет модули, выбранные через drill-down объекты модулей, контейнеры и разделители; шестеренка включает Search/Recent, задает количество колонок и строк grid и включает Edit Launchpad. В Edit mode можно перетаскивать tile-элементы, контейнеры, разделители и содержимое контейнеров; переносить элементы внутрь контейнеров и обратно; менять размер контейнера за нижний правый угол или через поля ширины/высоты; удаление контейнера возвращает его содержимое обратно в grid. Открытие модуля или объекта закрывает Launchpad, а команды оставляют окно открытым и показывают вывод. Ctrl+E переключает режим редактирования, Escape сначала выходит из edit/status-режима и только потом закрывает окно. Раскладка Launchpad, recent и глобальный hotkey хранятся локально для текущей машины; hotkey меняется в Settings > Shortcuts и применяется после перезапуска приложения.',
    ...
    telegraph_share_desc: 'В Share-диалоге можно публиковать заметки и сниппеты в Telegra.ph как snapshot-страницы, удобные для Telegram. Sync API готовит безопасный Telegra.ph-контент и сохраняет metadata страницы, а desktop native app напрямую выполняет publish/update в Telegra.ph, поэтому публикация работает даже когда сервер не может достучаться до Telegra.ph. Поскольку Telegra.ph не поддерживает табличные nodes, узкие Markdown-таблицы шириной до 32 экранных колонок остаются выровненным моноширинным текстом. Таблицы с непустыми строками превращаются в подписанные вертикальные записи, если они широкие или содержат ссылки; кликабельными становятся только безопасные адреса, а таблицы без строк или с пустыми строками остаются компактными. Ошибки открываются постоянной модалкой с копируемой диагностикой. Интерактивные HTML Cards упрощаются до безопасной ссылки на sandboxed HTML asset.',
    ...
    finance_facts_name: 'Импорт фактов Finance',
    finance_facts_desc: 'В модуле Финансы появился глобальный режим Facts для импорта банковских CSV-выгрузок Т-Банка. В Facts показывается отдельная левая панель фильтров дат вместо списка финансовых списков: можно фильтровать по точной дате, диапазону, месяцу или году, чтобы работать с большими выгрузками; фильтр месяца использует встроенный picker приложения и открывается надежно в desktop WebView. В реестре Facts на desktop и mobile есть заметный поиск по всем полям: датам, описанию, банковским полям, MCC/карте, суммам, состоянию lock/mapping и назначенной финансовой статье. Предпросмотр импорта показывает новые и дублирующиеся строки, расходы, поступления, диапазон дат и валюты до записи в базу. Ошибки импорта открываются в устойчивой модалке с кнопкой копирования; ошибки парсера включают колонку/значение и сырую CSV-строку, если ее удалось определить, а пустая дата платежа в строках Т-Банка берется из даты операции. Импортированные факты дедуплицируются по source/fingerprint, хранятся отдельно от плановых строк и могут назначаться вручную или через упорядоченные правила мэппинга. Назначение фактов и создание правил используют компактный иерархический selector статей: родительские группы видны как структура, но выбрать можно только терминальную статью без потомков. Из модалки Map можно создать предзаполненное правило по текущему факту и сразу применить его к существующим неназначенным фактам. Правила учитывают банковскую категорию, описание, MCC, направление и диапазоны сумм; применение правил по умолчанию обновляет только неназначенные факты, а переназначение уже назначенных требует явной опции и не затрагивает заблокированные факты, а существующие правила можно выбрать, отредактировать, сохранить и повторно применить из окна Rules. У каждого факта есть lock от правил, чтобы защитить ручное назначение от будущих прогонов. Факты, назначенные на строки, которые позже стали группами, помечаются красным alert и выводятся отдельным фильтром Group target всегда, когда такие факты есть глобально; клик по Group target сбрасывает дату и поиск перед показом этих фактов. Если терминальная строка с прямыми фактами или фактами оплат превращается в группу, Finance предлагает перенести эти прямые назначения на новый терминальный дочерний элемент. Факты, назначения, правила и batch-история синхронизируются через API; мобильные назначения повторно отправляются, пока сервер их не примет, окно Sync details на мобильном показывает pending rows, last debug и недавнюю row-level историю синхронизации, а в мобильном Facts есть постоянный переключатель Cards/Compact для широких и раскладных экранов. Public share финансового списка показывает только безопасные назначенные данные фактов: дату, описание, банковскую категорию, MCC, сумму и статью назначения.',

    sql_name: 'SQL Инструменты',
    sql_desc: 'Набор SQL-утилит из пяти инструментов: Парсер извлекает из запросов имена таблиц и ClickHouse-словари из семейства dictGet*, Анализатор генерирует анализ DDL по шаблонам, Макросинг оборачивает запросы в Jinja-макросы, Форматтер раскладывает многоэлементные WITH-блоки, SELECT-поля, верхнеуровневые условия WHERE/PREWHERE/HAVING и ClickHouse SETTINGS по строкам, Обфускатор заменяет чувствительные данные заглушками.',
...
function renderFeatures(container, lang) {
  ...
  const features = [
    ...
    { name: t(lang, 'finance_facts_name'), desc: t(lang, 'finance_facts_desc') },
...
function renderHotkeys(container, lang) {
  ...
  const hotkeys = [
    ...
    { key: 'Ctrl+Alt+Space',            desc: t(lang, 'hotkey_launchpad') },
    { key: 'Ctrl+Alt+Insert',           desc: t(lang, 'hotkey_whisper') },
