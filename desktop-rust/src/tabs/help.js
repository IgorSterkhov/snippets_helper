...
const i18n = {
  ...
  ru: {
    ...
    telegraph_share_desc: 'В Share-диалоге можно публиковать заметки и сниппеты в Telegra.ph как snapshot-страницы, удобные для Telegram. Sync API готовит безопасный Telegra.ph-контент и сохраняет metadata страницы, а desktop native app напрямую выполняет publish/update в Telegra.ph, поэтому публикация работает даже когда сервер не может достучаться до Telegra.ph. Поскольку Telegra.ph не поддерживает табличные nodes, корректные Markdown-таблицы публикуются как выровненный моноширинный текст без сырой pipe/separator-разметки. Ошибки открываются постоянной модалкой с копируемой диагностикой. Интерактивные HTML Cards упрощаются до безопасной ссылки на sandboxed HTML asset.',
