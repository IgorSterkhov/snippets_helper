...
const i18n = {
  en: {
    ...
    settings_desc: 'Configure sync, updates, display preferences, provider tokens, and per-tool options. Settings > AI stores the current sync user DeepSeek key and Telegram bot token on the server, can check DeepSeek balance, opens the DeepSeek usage cabinet in the browser, and manages Telegram chat pairing from the desktop UI. Server-appointed admins see an additional Users / Limits tab for checking user activity and storage usage, then adjusting each user media quota and max upload size. Admin assignment is intentionally not available in the UI and is done only by a server-side command.',
