...
const i18n = {
  en: {
    ...
    ai_name: 'AI Agent',
    ai_desc: 'Ask DeepSeek from a dedicated AI tab without storing the DeepSeek token on desktop or mobile; each sync API user saves their own server-side DeepSeek key in Settings > AI, and the key is never shown back in the app. Chat mode answers in the tab, while Command mode can open Tasks, Notes, and Snippets, create tasks, and add or complete task checkboxes through local app commands so normal sync propagates the changes. Desktop voice prompts reuse the configured Whisper/Deepgram flow; mobile voice prompts use Android speech recognition and require the APK build with microphone permission. Telegram bot support is server-side, deny-by-default, uses the DeepSeek key of the bound app user, and only works for chats explicitly bound by a server console command.',
  ...
  ru: {
    ...
    ai_name: 'AI Agent',
    ai_desc: 'Отдельная вкладка AI позволяет обращаться к DeepSeek без хранения DeepSeek-токена на десктопе или мобильном устройстве: каждый пользователь sync API сохраняет свой серверный DeepSeek-ключ в Settings > AI, и приложение никогда не показывает сохраненный ключ обратно. В режиме Chat агент отвечает во вкладке, а в режиме Command может открывать задачи, заметки и сниппеты, создавать задачи, добавлять или отмечать чекбоксы через локальные команды приложения, после чего обычная синхронизация разносит изменения. Голосовые запросы на десктопе используют настроенный Whisper/Deepgram flow; на мобильном используется Android speech recognition, поэтому нужна APK-сборка с разрешением на микрофон. Telegram-бот работает на сервере, по умолчанию запрещает неизвестные чаты, использует DeepSeek-ключ привязанного пользователя приложения и доступен только после явной привязки чата серверной консольной командой.',
