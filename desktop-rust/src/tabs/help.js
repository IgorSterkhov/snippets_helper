...
const i18n = {
  ...
  ru: {
    ...
    clickhouse_desc: 'Локальный справочник ClickHouse в группе DEV. Модуль хранит официальные raw Markdown документы ClickHouse в локальной SQLite базе, обнаруживает актуальный каталог sql-reference/functions через GitHub contents metadata и разбивает большие страницы на секции/функции. При открытии модуль загружает только дерево навигации через async IPC; если локальный кэш читается медленно, показывается состояние долгой загрузки, а остальная часть приложения остается доступной. Выбор страницы открывает легкое оглавление только с метаданными и короткими выдержками, активная страница раскрывается в левой навигации, а полный Markdown body загружается и рендерится только после выбора одной секции. Поиск тоже работает на уровне секций/функций, поэтому большие страницы функций возвращают нужный блок, а не всю статью. Кнопка Update docs вручную обновляет локальный кэш из официального репозитория документации ClickHouse и показывает нативный прогресс с процентом, оставшимися страницами, временем выполнения, итоговой сводкой, временем последнего обновления и счетчиками страниц/секций. Changelog показывает добавленные, измененные, удаленные или неудачно загруженные источники последних обновлений.',
