...
const i18n = {
  en: {
    ...
    clickhouse_desc: 'A local ClickHouse documentation browser in the DEV group. It stores a curated set of official ClickHouse raw Markdown docs in the local SQLite database and parses large pages into section/function blocks. The module opens by loading only the navigation tree through async IPC; if the local cache is slow, it shows a slow-load state while the rest of the app stays usable. Selecting a page opens a lightweight section index, expands the active page in the left navigation, and selecting a section renders only that block. Search also works at section/function granularity so large function pages return the relevant block instead of the whole article. The Update docs button refreshes the local cache manually from the official ClickHouse docs repository and shows native progress with percentage, remaining pages, elapsed time, final summary, last update time, and page/section counts. Changelog shows added, changed, removed, or failed sources from recent update runs.',
  ...
  ru: {
    ...
    clickhouse_desc: 'Локальный справочник ClickHouse в группе DEV. Модуль хранит курируемый набор официальных raw Markdown документов ClickHouse в локальной SQLite базе и разбивает большие страницы на секции/функции. При открытии модуль загружает только дерево навигации через async IPC; если локальный кэш читается медленно, показывается состояние долгой загрузки, а остальная часть приложения остается доступной. Выбор страницы открывает легкое оглавление секций, активная страница раскрывается в левой навигации, а выбор секции рендерит только этот блок. Поиск тоже работает на уровне секций/функций, поэтому большие страницы функций возвращают нужный блок, а не всю статью. Кнопка Update docs вручную обновляет локальный кэш из официального репозитория документации ClickHouse и показывает нативный прогресс с процентом, оставшимися страницами, временем выполнения, итоговой сводкой, временем последнего обновления и счетчиками страниц/секций. Changelog показывает добавленные, измененные, удаленные или неудачно загруженные источники последних обновлений.',
